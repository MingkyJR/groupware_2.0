...
<% request.setCharacterEncoding("utf-8"); %>
	<form name="rowSizeFrm" id="rowSizeFrm"
		action="/document/returnDocument.do" method="get">
	</form>
		<table border="1">
		<thead>
			<tr>
			...
			<tr>
			 	<td colspan="8" style="text-align: center;">
					<%-- JSTL if조건문: 이전출력--%> <c:if test="${documentPage.startPage>5}">
						<a	href="/document/returnDocument.do?pageNo=${documentPage.startPage-5}&rowSize=${rsize}">prev</a>
					</c:if> <!-- JSTL forEch조건문: 페이지번호출력  --><c:forEach var="pNo"
						begin="${documentPage.startPage}" end="${documentPage.endPage}">
						<a href="/document/returnDocument.do?pageNo=${pNo}&rowSize=${rsize}">${pNo}</a>
					</c:forEach> <!-- JSTL if조건문: 다음출력 --> 
					<c:if test="${documentPage.endPage<documentPage.totalPages}">
						<a
							href="/document/returnDocument.do?pageNo=${documentPage.startPage+5}&rowSize=${rsize}">next</a>
					</c:if>
				</td> 
			</tr>
